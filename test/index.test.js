...
describe('TypingMonitor', () => {
  it('exports sub classes', () => {
    expect(
      new TypingMonitor({ wait: 500 })
    ).toBeInstanceOf(StaticMonitor);

    expect(
      new TypingMonitor.Static({ wait: 500 })
    ).toBeInstanceOf(StaticMonitor);

    expect(
      new TypingMonitor.Listener({
        wait: 500,
    ...
    expect(
      new TypingMonitor.Global({ wait: 500 })
    ).toBeInstanceOf(GlobalMonitor);
  });
});
